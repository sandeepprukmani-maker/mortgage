...
def customer_to_payload(customer_obj: Any, base_payload: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    # Merge customer-specific values
    ...
    # Normalize loanTypeIds (handles ["0, 2, 4"] or "0,2,4")
    lt_ids = to_str_list(merged.get("loanTypeIds"), default=["0"])

    # IMPORTANT: correct casing = purposeTypeId (not purposeTypeID)
    purpose_type_id = str(merged.get("purposeTypeId", "3"))
    if purpose_type_id == "3" and len(lt_ids) > 1:
        lt_ids = [lt_ids[0]]

    # Doc says these are unsupported — never send
    ...
    return {
        ...
        "monthsOfBankStatementsId": merged.get("monthsOfBankStatementsId", None),
        "monthsOfReservesId": merged.get("monthsOfReservesId", None),
        ...
        "loanTermIds": to_str_list(merged.get("loanTermIds"), default=["4"]),
