...
def post_price_quote(access_token: str, payload: dict) -> requests.Response:
    ...
    logger.info("Posting payload to UWM (normalized) keys=%s",
...
def analyze_next_customer(cache_key):
    try:
        ...
        for buydown in buydown_scenarios:
            ...
            logger.info("UWM response status=%s body_snip=%s",
...
def get_accurate_buydown():
    try:
        ...
        buydown_percent_raw = data.get("buydown_percent", 0)
        buydown_percent = float(
            buydown_percent_raw) if buydown_percent_raw is not None else 0.0
        base_payload = data.get("payload", {}) or {}
        ...
        for item in quote_body.get("validQuoteItems", []):
            if item.get("actualTermYears") != term_years or item.get(
            ...
            for pp in item.get("quotePricePoints", []):
                ...
                if abs(rate_val - target_rate) < 0.01:
                    ...
                    return jsonify({
                        ...
                        "payment":
                        mp,
                        "savings": (customer.current_monthly_payment -
                                    mp) if mp is not None else None,
                        "credit_cost":
                        safe_float((pp.get("finalPriceAfterOriginationFee")
                                    or {}).get("amount")),
                        "is_exact":
                        True
                    })
