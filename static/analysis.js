...
function displaySummaryTable(data) {
  ...
  // Collect highlighted (closest to target) rates from each product
  const summaryData = {};
  ...
  for (const scenario of data.scenarios || []) {
    ...
    for (const product of scenario.products || []) {
      ...
      // Find the highlighted rate (closest to target)
      ...
      if (highlightedRate) {
        ...
        // For buydowns, extract Year 1 data from buydown_breakdown if available
        let displayRate = highlightedRate.interest_rate;
        let displayPayment = highlightedRate.monthly_payment;
        let displaySavings = highlightedRate.monthly_savings;
        let displayCost = highlightedRate.credit_cost || 0;
        let yearLabel = '';
        let hasYear1Data = false;
        let isExact = undefined;

        if (buydownType !== 'None') {
          if (highlightedRate.buydown_breakdown && highlightedRate.buydown_breakdown.year1) {
            // Use Year 1 data from the breakdown
            displayRate = highlightedRate.buydown_breakdown.year1.rate;
            displayPayment = highlightedRate.buydown_breakdown.year1.payment;
            displaySavings = highlightedRate.buydown_breakdown.year1.savings;
            displayCost = highlightedRate.buydown_breakdown.year1.credit_cost || displayCost;
            yearLabel = 'Year 1';
            hasYear1Data = true;
            isExact = highlightedRate.buydown_breakdown.year1.is_exact;
          } else if (highlightedRate.buydown_breakdown) {
            // If buydown_breakdown exists but no year1, show N/A for year 1
            displayRate = 'N/A';
            displayPayment = 'N/A';
            displaySavings = 'N/A';
            displayCost = 'N/A';
        ...
        summaryData[term].push({
          ...
          cost: displayCost,
          hasBuydown: buydownType !== 'None',
          yearLabel: yearLabel,
          hasYear1Data: hasYear1Data,
          isExact: isExact
        });
      }
    }
  ...
  // Generate HTML for each term table
  container.innerHTML = sortedTerms.map(term => `
    <div class="form-section" style="margin-bottom: 2rem;">
            ...
            <tbody>
              ${summaryData[term].map((item, idx) => `
                <tr style="${idx % 2 === 0 ? 'background-color: #f9fafb;' : ''}">
                  <td>
                    <strong>${escapeHtml(item.buydownType)}</strong>
                  </td>
                  <td>${escapeHtml(item.productName)}</td>
                  <td>
                    <strong>${formatPercent(item.rate)}</strong>
                    ${item.yearLabel ? `<div style="font-size: 0.75rem; color: #10b981; margin-top: 0.25rem;">${item.yearLabel}${item.isExact === false ? ' (approx)' : ''}</div>` : ''}
                    ${item.hasBuydown && item.yearLabel && item.isExact === false ? `<button class="btn btn-sm btn-outline-primary" style="margin-top: 0.25rem;" onclick="getAccurateBuydown(this, '${data.customer.customer_key}', '${item.productName}', '${term}', '${item.buydownType}', '${item.rate}', '${item.payment}', '${item.savings}', '${item.cost}')">Get Accurate</button>` : ''}
                  </td>
                  <td>
                  ...
                  <td class="${item.cost < 0 ? 'text-success' : ''}">
      ...
      ${summaryData[term].some(item => item.hasBuydown && item.hasYear1Data) ? `
        <div style="margin-top: 1rem; padding: 0.75rem; background: #dcfce7; border-left: 3px solid #10b981; font-size: 0.85rem; color: var(--text-secondary);">
          <strong>✓ Note:</strong> For buydown products (1-0 LLPA, 2-1 LLPA), the rates, payments, and savings shown are <strong>Year 1 values</strong> from actual API responses.
          These are the temporary discounted rates customers will pay in the first year.
          To see the full year-by-year breakdown including Years 2 and 3+, expand the "Years" details in the "All Available Options" section below.
        </div>
      ` : ''}
    </div>
  `).join('');
...
window.getAccurateBuydown = async function(button, customerKey, productName, termYears, buydownType, rate, payment, savings, cost) {
  ...
  try {
    ...
    let buydownPercent = 0;
    if (buydownType.includes('2-1')) buydownPercent = 2;
    else if (buydownType.includes('1-0')) buydownPercent = 1;
    ...
    const resp = await fetch('/api/buydown/accurate', {
      ...
      body: JSON.stringify({
        ...
        buydown_percent: buydownPercent,
        payload: payload
      })
    });
    ...
    if (result && result.is_exact) {
      ...
      button.parentElement.parentElement.querySelectorAll('td')[5].innerHTML = `${formatCurrency(result.credit_cost)} ⭐<div style="font-size: 0.75rem; color: #10b981; margin-top: 0.25rem;">Year 1</div>`;
      button.remove();
    } else {
