...
async function deleteSelectedPayload() {
  const select = document.getElementById('payloadSelect');
  const payloadName = select.value;
  ...
  try {
    await deletePayload(payloadName);
    await loadPayloadList();
    document.getElementById('builderForm').reset();
...
async function handleSavePayload(e) {
  e.preventDefault();
  ...
  try {
    await savePayload(payloadName, payload);
    await loadPayloadList();
