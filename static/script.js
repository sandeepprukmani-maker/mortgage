  ...
  function displayResults(data, section, container) {
    if (!data || data.length === 0) {
      ...
      data.forEach(customerResult => {
        ...
        // Group scenarios by term across all scenarios
        const termsMap = new Map();
        (customerResult.scenarios || []).forEach(scenario => {
          (scenario.termGroups || []).forEach(tg => {
            const term = tg.actualTermYears;
            ...
            termsMap.get(term).buydowns.push({
              label: scenario.scenario,
              buyDownAliasID: scenario.buyDownAliasID,
        ...
        const sortedTerms = Array.from(termsMap.values()).sort((a, b) => (a.term || 99) - (b.term || 99));

        const termsHtml = sortedTerms.map(termGroup => {
          const buydownsHtml = termGroup.buydowns.map(bd => {
            if (bd.error) return `<div class="error-msg">${bd.label}: ${bd.error}</div>`;
            
            const productsHtml = bd.products.map(prod => {
              const rate = prod.rateValue || 0;
              let breakdownHtml = '';
              
              if (bd.buyDownAliasID === '1-0 LLPA') {
                breakdownHtml = `<div class="buydown-breakdown">Yr 1: ${(rate - 1).toFixed(3)}% | Yr 2+: ${rate.toFixed(3)}%</div>`;
              } else if (bd.buyDownAliasID === '2-1 LLPA') {
                breakdownHtml = `<div class="buydown-breakdown">Yr 1: ${(rate - 2).toFixed(3)}% | Yr 2: ${(rate - 1).toFixed(3)}% | Yr 3+: ${rate.toFixed(3)}%</div>`;
              }

              return `
                <tr class="${prod.isClosest ? 'highlight-closest' : ''}">
                  <td>
                    ${rate.toFixed(3)}%
                    ${prod.isClosest ? ' <span class="badge badge-yellow">Target</span>' : ''}
                    ${breakdownHtml}
                  </td>
            ...
            return `
              <div class="buydown-section">
                <div class="buydown-title">${bd.label}</div>
          ...
          return `
            <div class="term-group">
              <div class="term-header" onclick="this.nextElementSibling.classList.toggle('hidden')">
                <span style="font-weight: 700; color: var(--primary-color);">${termGroup.term} Year Term Options</span>
