document.addEventListener('DOMContentLoaded', () => {
  ...
  window.analyzeCustomer = async customerId => {
    ...
    let body;
    if (payloadTemplate) {
      // Extract payload and excluded fields
      ...
      // Override customer-specific fields ONLY if they exist in template
      if ('borrowerName' in mergedPayload) {
        mergedPayload.borrowerName = customer.name;
      }
      if ('monthlyIncome' in mergedPayload) {
        mergedPayload.monthlyIncome = customer.monthly_income;
      }
      if ('creditScore' in mergedPayload) {
        mergedPayload.creditScore = customer.credit_score;
      }
      if ('loanAmount' in mergedPayload) {
        mergedPayload.loanAmount = customer.remaining_balance;
      }
      if ('appraisedValue' in mergedPayload) {
        mergedPayload.appraisedValue = customer.property_value;
      }
      if ('propertyZipCode' in mergedPayload) {
        mergedPayload.propertyZipCode = customer.property_zip;
      }
      if ('propertyState' in mergedPayload) {
        mergedPayload.propertyState = customer.property_state;
      }
      if ('propertyCounty' in mergedPayload) {
        mergedPayload.propertyCounty = customer.property_county || '';
      }

      // Add currentMonthlyPayment for backend savings calculation
      mergedPayload.currentMonthlyPayment = customer.current_monthly_payment;

      // ✅ Fix: Ensure monthsOfReservesId is a string if it exists as a number
  ...
  loadJsonBtn.addEventListener('click', () => {
    ...
    try {
      ...
      // If it's the {payload, excludedFields} format
      const payload = imported.payload || imported;
      const excludedFields = imported.excludedFields || [];

      // Update toggles - First, handle selection based on payload keys or explicit exclusions
      const toggles = builderForm.querySelectorAll('.field-toggle');
      toggles.forEach(toggle => {
        const fieldName = toggle.dataset.field;
        if (!imported.excludedFields) {
          // If loading a bare payload, only check fields present in JSON
          toggle.checked = (fieldName in payload);
        } else {
          // If explicit exclusions exist, use them
          toggle.checked = !excludedFields.includes(fieldName);
        }
      });
      ...
      alert('JSON loaded into form successfully!');
    } catch (e) {
  ...
  function displayResults(data) {
    ...
    data.forEach(customerResult => {
      ...
      // Build HTML for each buydown scenario
      const scenariosHtml = (customerResult.scenarios || [])
        .map(scenario => {
          // Build term groups for this scenario
          const termGroupsHtml = (scenario.termGroups || [])
            .map(termGroup => {
              const productsHtml = termGroup.products
                .map(
                  prod => `
                  <tr>
              ...
              return `
                <div class="term-group">
                  <h5>${termGroup.mortgageProductName} (${termGroup.actualTermYears} years)</h5>
                  <small>${termGroup.mortgageProductAlias || ''}</small>
          ...
          const totalOptions = (scenario.termGroups || []).reduce(
          ...
          return `
            <div class="scenario-column">
              <div class="scenario-header">
                ...
                <span class="badge">${totalOptions} options</span>
              </div>
      ...
      card.innerHTML = `
        ...
        <h4 style="margin-top: 1.5rem;">Buydown Comparison</h4>
